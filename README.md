...
생성되는 PDF 리포트에는 다음 내용이 포함됩니다:

1. **표지 (Cover)**
   - 제목: Monthly Expense Report
   - 분석 기간
   - 생성 일시
...
3. **카테고리별 분석 (Category Analysis)**
   - 지출 비율 파이 차트
   - 카테고리별 금액 및 비율 테이블

4. **월별 추이 (Monthly Trend)**
   - 월별 수입/지출 막대 그래프

5. **예산 현황 (Budget Status)** - 예산 설정 시
   - 카테고리별 예산 대비 실적
   - 사용률 및 상태

6. **거래 내역 (Transaction Details)**
   - 최근 20건 상세 내역
   - 날짜, 적요, 금액, 카테고리

7. **푸터 (Footer)**
   - 생성 정보
   - Powered by Expense Analyzer

---

**⭐ 이 프로젝트가 # 💰 개인 가계부 분석기 (Expense Analyzer)
